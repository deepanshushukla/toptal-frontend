            ...
            <div className='filtersConatiner'>
                {filterConfig.map((item)=>{
                    if(item.type === DROPDOWN){
                        return <DropdownFilter {...item} selectedFilter={selectedFilter} onFilterSelect={onFilterSelect}/>
                    }
                    if(item.type === RANGE){
                        return <RangeFilter {...item} selectedFilter={selectedFilter} onFilterSelect={onFilterSelect} />
                    }
                })}
                {Object.keys(selectedFilter).length ? <Button type='link' onClick={clearAllFilter} icon={<CloseCircleOutlined />}>Clear All Filters</Button> : null }
            </div>
