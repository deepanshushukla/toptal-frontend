...
const RangeFilter = ({placeHolder,onFilterSelect,selectedFilter,dataKey, labelFormatter}) => {
    let min, max ;
    ...
    const hasClickedOut = useOutsideClickAlert(wrapperRef);
    useEffect(()=>{
        if(hasClickedOut && visible ){
            toggleOverlay();
        }
    },[hasClickedOut]);
    useEffect(()=>{
    ...
    return (
            <div className='rangeFilterConatiner' ref={wrapperRef}>
                    <Button onClick={toggleOverlay}>
                        {selectedFilter && selectedFilter[dataKey] &&
                            (min || max) ?
